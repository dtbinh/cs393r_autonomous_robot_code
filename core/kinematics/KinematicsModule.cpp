...
void KinematicsModule::calculatePose() {
  calculatePose(joints_->values_, sensors_->values_, robot_info_->dimensions_.values_, body_model_);
  ForwardKinematics::calculateCoM(body_model_->abs_parts_,body_model_->center_of_mass_, robot_info_->mass_calibration_);
}

void KinematicsModule::calculatePose(float* joints, float* sensors, float* dimensions, BodyModelBlock* body_model) {

  ForwardKinematics::calculateRelativePose(joints, 0, 0, body_model->rel_parts_, dimensions);

  Pose3D base = ForwardKinematics::calculateVirtualBase(sensors, body_model->rel_parts_);
  // Calculate absolute pose from the virtual base
  ForwardKinematics::calculateAbsolutePose(base, body_model->rel_parts_, body_model->abs_parts_);
