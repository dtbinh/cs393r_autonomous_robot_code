...
class Playing(StateMachine):
    ...
    def run(self):
      ...
      if(ball.seen):
        ...
        if( seen_times >= 10 )
          ...
          if(ball_distance < 175 and ball_image_x > 177 and ball_image_x < 207): #stop and prepare kick
          ...
          elif(ball_distance > 450 or ball_distance < 350 or Approaching_state == 1): #direct walking area
            ...
            PID_walk_velocity = ( last_PID_walk_velocity_0 + last_PID_walk_velocity_1 + last_PID_walk_velocity_2 )/3.0
            if( PID_walk_velocity > 0.7):
              PID_walk_velocity = 0.7

            if(ball_distance > 450):
            ...
            if(goal.seen):
              if(goal.imageCenterX < 175 or goal.imageCenterX > 145):
                Approaching_state = 1
              else:
                commands.setWalkVelocity(0 , 0.15 , 0.15)
                memory.speech.say("three")
            else:
                commands.setWalkVelocity(0 , 0.25 , 0.25)
      ...
      ball_if_seen_counter = (ball_if_seen_counter + 1)%20;      
        #Approaching_state = 0
  ...
  def setup(self):
    ...
    self.trans(stand, C, pid_walk, C, sit, C, off)
