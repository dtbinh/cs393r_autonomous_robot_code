...
public:
    ...
    MCLParticleFilter(StateTransitionMatrix A_matrix, ControlMatrix B_matrix, MeasurementCovarianceMatrix Q_matrix, WhiteCovarianceVector N_matrix )
    {
...
private:
    ...
    void low_variance_sampler()
    {
      ...
      //for( i = 0 ; i < NumParticle ; i++){cout << "c(" << i <<") = " << setprecision(15) << c(i) << endl ;}
        ...
        double thres =  (rand()*1.0/RAND_MAX)/NumParticle;
        for( i = 0 ; i < NumParticle ; i++)
        {
            while( thres > c(j)) ++j;
            X.col(i) = X_bar.col(j);
            W(i) = 100;
            thres += 1.0/NumParticle;
        }
